...
release:
	@echo "${GIT_TAG}" | grep -q "${VERSION}" || (echo "Software version in APITaxi_front/__init__.py does not match the tag on HEAD. Please update __version__, and tag the current commit with \`git tag <version>\`." ; exit 1)
